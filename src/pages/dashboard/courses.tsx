...
function CoursesDashboard({
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const baseUrl = `http://192.168.1.137:8080/api/courses`
  ...
  const onClickCourse = (id: number) => async (e: any) => {
    ...
    const rawCourse = await fetch(`http://192.168.1.137:8080/api/courses/${id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        authorization: `Bearer ${authorization}` || '',
  ...
  const onDeleteCourse = async () => {
    ...
    try {
      await fetch(`http://192.168.1.137:8080/api/courses/${editedId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${authorization}` || '',
